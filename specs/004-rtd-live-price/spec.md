...
- Q: When both live price and day-ahead forecast are enabled for a subentry, does the coordinator make a separate RTD call (alongside the existing PRSL/NRSL call), or does RTD replace/share the day-ahead call? → A: Separate calls — RTD call added for live price; existing PRSL/NRSL day-ahead call continues unchanged for forecast.
- Q: Should the coordinator poll at 5-minute intervals always (for any configured subentry) or only dynamically when live price is enabled? → A: Always 5 min — once any market-node subentry exists, the coordinator polls at 5-minute intervals regardless of live price toggle state.
- Q: Which RTD price field should the live price sensor display (`price`, `price6s`, or `price60s`)? → A: `price` — the standard dispatch price field, consistent with all other schedule types.
- Q: What exact `back` parameter value should be used for the RTD API call? → A: `back=3` — RTD publishes at 5-minute dispatch intervals; `back=3` returns ~15 minutes of history, providing a safe margin while minimising data transfer. The live price sensor displays the most recently dispatched interval (`max` by `trading_datetime`), which is also the last entry in the `history` attribute.
- Q: Should the `forward` parameter be included in the RTD API call? → A: Omit `forward` (pass `None`) — RTD is back-looking; no forward window is needed or valid.
...
- A single RTD API call with `back=3` and no `forward` parameter is sufficient to guarantee the most recently dispatched period is included in the response, even if the poll does not land exactly at the moment of RTD publication. RTD publishes prices at 5-minute dispatch intervals (not 30-minute trading periods), so `back=3` returns approximately 15 minutes of history. The live price sensor selects the period with the largest `trading_datetime ≤ utcnow()` — i.e., the most recently dispatched interval — as its state value. This means `native_value` always matches `history[-1]["price"]`.
...
- Reason: Corrected Assumption 4 — RTD publishes at 5-minute dispatch intervals (not 30-minute trading periods), making `back=3` ≈ 15 min of history rather than 90 min. Updated clarification Q4 accordingly. `_extract_live_price_payload` was changed from a 30-minute window lookup to `max(past, key=trading_datetime)` to always select the most recently dispatched interval; `native_value` now always matches `history[-1]["price"]`.
