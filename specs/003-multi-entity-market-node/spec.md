...
2. **Given** a configured live price sensor, **When** the coordinator fetches new forecast data (PRSL or PRSS via `forward` parameter), **Then** the sensor value updates to the current trade period's price from the forecast response. Attributes `timestamp`, `trading_period`, `node`, and `schedule` are updated; no `forecast` attribute is set on the live sensor.
...
- **FR-003**: System MUST create a live current-price sensor when the user enables the "Current electricity price" option. The live price sensor MUST source its current price from the **current trade period's price within the day-ahead forecast schedule response** (`forward=48`, using PRSL or NRSL based on configured forecast type). This shares a single coordinator fetch with the day-ahead forecast sensor: the current trade period's price becomes the live sensor state. The live sensor exposes only `timestamp`, `trading_period`, `node`, and `schedule` as attributes — it does NOT expose a `forecast` attribute. Forecast price data (strictly future, not-yet-started periods) is the responsibility of `DayAheadForecastSensor`.
...
- **FR-007**: System MUST allow users to configure history retention (6h, 12h, or 24h) for forecast sensors. The configured retention defines the forecast-history API lookback (`back`) window, and all returned trading periods that have already started (elapsed or currently active) MUST be stored in the sensor `history` attribute. The `forecast` attribute MUST contain only strictly future (not-yet-started) periods.
...
- **FR-017**: The day-ahead forecast sensor MUST expose up to 48 strictly future (not-yet-started) trade periods in its `forecast` attribute, and MUST include the currently active trade period plus all fully elapsed periods returned by the retention-defined API lookback in `history`. The currently active trade period (started ≤ now, ends > now) belongs in `history`, not `forecast`.
...
- **Live Price Sensor**: A sensor entity showing the current wholesale electricity price for a market node. The current price is sourced from the **current trade period's price in the day-ahead forecast schedule response** (`forward=48`, PRSL or NRSL based on forecast type). Attributes: `timestamp`, `trading_period`, `node`, `schedule`. Does NOT include a `forecast` attribute — forecast data belongs to `DayAheadForecastSensor`.
...
- **History Retention**: The lookback window for forecast history data. The configured retention value is used as the API `back` window for forecast history, and all returned trading periods that have already started (elapsed or currently active) are stored in `history`. The currently active (in-progress) trade period is part of `history`, not `forecast`. For example, a 24h retention setting keeps the previous 24h of trading periods in history.
...
- Forecast history retention defines the forecast API lookback (`back`) window used by the integration and governs which trading periods are stored in the sensor `history` attribute; the currently active trade period is included in `history` (not `forecast`); it does not alter provider-side retention.
...
- Q: What API schedule and query pattern should the Live Price Sensor use? → A: Forecast schedule (`forward` parameter) — the current trade period's price is extracted from the forecast response; this shares a single coordinator fetch with the day-ahead forecast sensor (current period price = sensor state). The live sensor exposes only `timestamp`, `trading_period`, `node`, `schedule` — no `forecast` attribute.
...
- Q: How should 003 handle existing 002 config entries on upgrade — clean removal, automatic migration, or coexistence? → A: Automatic migration — 003 reads and converts existing 002 config entries to the 003 structure; configuration is migrated and prices are fetched post-migration in the configured display unit; entity IDs preserved where possible; changed IDs logged as warnings.
- Q: On HA restart, which sensor types should restore their last known state vs start unavailable? → A: Live price sensor only restores last known state (single value still useful as context); forecast and accounting sensors start unavailable but prime from coordinator data fetched during entry setup — see Session 2026-05-27 for precise startup-fetch behaviour.
...
- Q: Should `LivePriceSensor.extra_state_attributes` include a `forecast` list of remaining future trade periods from the day-ahead response? → A: No — `LivePriceSensor` exposes only `timestamp`, `trading_period`, `node`, `schedule`. Forecast data (remaining future periods) is the exclusive responsibility of `DayAheadForecastSensor`.
- Q: After migration to 003, should the `"sensor"` subentry type remain registered in `async_get_supported_subentry_types`? → A: No — deregister the legacy `"sensor"` subentry type; only `"market_node"` is available post-migration. Allowing new 002-style subentries to be created after migration is prohibited.
- Q: Should `DayAheadForecastSensor` / `IntradayForecastSensor` `forecast` attribute include the currently active (in-progress) trade period? → A: No — the currently active trade period belongs in `history`; `forecast` contains only strictly future (not-yet-started) periods. `native_value` remains the current period's price.
- Q: After HA restart, when should non-RestoreEntity sensors (settled price, forecast, per-period import/export) first pick up coordinator data? → A: Prime at registration — sensors call `_handle_coordinator_update()` in `async_added_to_hass` if coordinator data already exists (fetched during `async_config_entry_first_refresh()`), making them available immediately after entry setup. "First coordinator poll" in earlier clarifications means the setup poll, not the next scheduled 30-minute interval.

### Revision: Implementation Sync 2026-05-27

- **FR-003**: Removed erroneous `forecast` attribute requirement from `LivePriceSensor`; updated FR-003, Key Entities, US1 Acceptance Scenario 2, and data-model.md.
- **FR-019**: Added explicit prohibition on registering the `"sensor"` subentry type post-migration.
- **data-model.md**: Corrected `DayAheadForecastSensor.native_value` description to "current trade period" (matching implementation and FR-003).
- **contracts/sensor-platform.md**: Corrected Interim fetch `back` value from fixed `48` to `accounting_retention_hours × 2`.
- **tasks.md**: Marked T035–T040 complete; added T045–T049 remediation tasks for remaining drift.
