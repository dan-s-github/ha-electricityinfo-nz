...
- **Live Price Sensor**: A sensor entity showing the current wholesale electricity price for a market node. The current price is sourced from the **current trade period's price in the day-ahead forecast schedule response** (`forward=48`, PRSL or NRSL based on forecast type). The same single coordinator fetch serves both the current price (sensor state = current trade period) and the forecast attribute (remaining future periods).
